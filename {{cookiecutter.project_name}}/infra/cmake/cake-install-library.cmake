...
#
# name
#   Logical package name (e.g. "cake.utility").
...
#   "[===[cake.inplace_vector 1.0.0]===] [===[cake.scope 0.0.1 EXACT]===] fmt"
#
# NAMESPACE (optional)
...
# ----------------
#
# CAKE_INSTALL_CONFIG_FILE_PACKAGES
...
function(cake_install_library name)
    # ----------------------------
    # Argument parsing
    # ----------------------------
    ...
    if(NOT CAKE_INSTALL_TARGETS)
        message(
            FATAL_ERROR
            "cake_install_library(${name}): TARGETS must be specified"
        )
    endif()
    ...
    if(CMAKE_SKIP_INSTALL_RULES)
        message(
            WARNING
            "cake_install_library(${name}): not installing targets '${CAKE_INSTALL_TARGETS}' due to CMAKE_SKIP_INSTALL_RULES"
        )
        return()
    endif()
    ...
    # Defaults
    # ----------------------------
    if(NOT CAKE_INSTALL_NAMESPACE)
        set(CAKE_INSTALL_NAMESPACE "cake::")
    endif()

    if(NOT CAKE_INSTALL_EXPORT_NAME)
        set(CAKE_INSTALL_EXPORT_NAME "${name}-targets")
    endif()

    if(NOT CAKE_INSTALL_DESTINATION)
        set(CAKE_INSTALL_DESTINATION
            "${CMAKE_INSTALL_DATADIR}/${name}/modules"
        )
    endif()

    string(REPLACE "cake." "" install_component_name "${name}")
    message(
        VERBOSE
        "cake-install-library(${name}): COMPONENT '${install_component_name}'"
    )

    # --------------------------------------------------
    # Install each target with all of its file sets
    # --------------------------------------------------
    foreach(_tgt IN LISTS CAKE_INSTALL_TARGETS)
        if(NOT TARGET "${_tgt}")
            message(
                WARNING
                "cake_install_library(${name}): '${_tgt}' is not a target"
            )
            continue()
        endif()
        ...
        if(NOT name_parts_length EQUAL 2)
            message(
                FATAL_ERROR
                "cake_install_library(${name}): expects a name of the form 'cake.<name>', got '${_tgt}'"
            )
        endif()
        ...
        message(
            VERBOSE
            "cake_install_library(${name}): EXPORT_NAME ${component_name} for TARGET '${_tgt}'"
        )
        ...
        if(_available_header_sets)
            message(
                VERBOSE
                "cake-install-library(${name}): '${_tgt}' has INTERFACE_HEADER_SETS=${_available_header_sets}"
            )
        ...
        if(_module_sets)
            message(
                VERBOSE
                "cake-install-library(${name}): '${_tgt}' has INTERFACE_CXX_MODULE_SETS=${_module_sets}"
            )
            install(
                TARGETS "${_tgt}"
                EXPORT ${CAKE_INSTALL_EXPORT_NAME}
                ...
                FILE_SET ${_module_sets}
                    DESTINATION "${CAKE_INSTALL_DESTINATION}"
    ...
    install(
        EXPORT ${CAKE_INSTALL_EXPORT_NAME}
        NAMESPACE ${CAKE_INSTALL_NAMESPACE}
    ...
    #   2. Allow-list CAKE_INSTALL_CONFIG_FILE_PACKAGES (if defined)
    #   3. Default: ON
    # ----------------------------------------
    ...
    if(DEFINED CAKE_INSTALL_CONFIG_FILE_PACKAGES)
        if(NOT "${name}" IN_LIST CAKE_INSTALL_CONFIG_FILE_PACKAGES)
    ...
    # ----------------------------------------
    set(_cake_find_deps "")
    foreach(dep IN LISTS CAKE_INSTALL_DEPENDENCIES)
        message(
            VERBOSE
            "cake-install-library(${name}): Add find_dependency(${dep})"
        )
        string(APPEND _cake_find_deps "find_dependency(${dep})\n")
    endforeach()
    set(CAKE_INSTALL_FIND_DEPENDENCIES "${_cake_find_deps}")

    # ----------------------------------------
    # Generate + install config files
    # ----------------------------------------
    if(_install_config)
        set(CAKE_INSTALL_BASE_PKG_NAME ${name})
        ...
        message(
            WARNING
            "cake-install-library(${name}): Not installing a config package for '${name}'"
        )
    endif()
endfunction()
